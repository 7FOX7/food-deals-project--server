...
enum Unavailable {
    ...
    SecondaryPrice = "[COULD NOT GET PRODUCT SECONDARY PRICE]"
}
            ...
            try {
                ...
                const currentProducts = await page.$$eval("div.tile-product", (divProducts, getProductCategory__funcBody) => {
                    ...
                    return divProducts.flatMap(div => {
                        ...
                        let primaryPrice = div.querySelector("span.price-update")?.textContent?.replaceAll("$", "") ?? Unavailable.PrimaryPrice
                        let secondaryPrice = div.querySelector("div.pricing__secondary-price span")?.textContent ?? Unavailable.SecondaryPrice
                        const units = div.querySelector("span.head__unit-details")?.textContent ?? Unavailable.Units
