...
const getData = async (): Promise<Products> => {
    ...
    const handle = async () => {
        try {
            ...
            const response = await page.goto(`https://haisue.ca/collections/all-products-and-in-stock`, {
            ...
            async function addProducts() {
                try {
                    ...
                    await page.waitForSelector("div.main-products-grid__results", { visible: true })
                    ...
                    const currentProducts = await page.$$("div.main-products-grid__results ul li.js-pagination-result")
                    // iterate through each product
                    for (const currentProduct of currentProducts) {
                        // get the title 
                        const title = await currentProduct.evaluate(el => el.querySelector("p.card__title a")?.textContent?.trim()) ?? Unavailable.Title
                        // get price part 1
                        const pricePart1 = await currentProduct.evaluate(el => el.querySelector("div.price__default span")?.firstChild?.textContent?.trim())
                        // get price part 2 
                        const pricePart2 = await currentProduct.evaluate(el => el.querySelector("div.price__default span sup")?.textContent?.trim())
                        // build the price
                        const primaryPrice = pricePart1 && pricePart2 ? `${pricePart1}.${pricePart2}` : Unavailable.PrimaryPrice
                        // get units match 
                    ...
                    await page.waitForSelector("div.main-products-grid__results ul", { visible: true })
                    // get the number of products for the current page
                    const productsCount = await page.$eval("div.main-products-grid__results ul", div => div.querySelectorAll("li.js-pagination-result").length)
                    ...
                    await page.waitForSelector("a.pagination__arrow--next", { visible: true })
                    // click on the `next` button and wait for navigation
                    await Promise.all([
                        page.click("a.pagination__arrow--next"), 
                        // page.waitForNavigation()         this won't work
                    ])
                    // wait for a bit to make sure we navigated to the next page
            ...
            console.error("Something went wrong (Thai Binh Asian Food): " + err)
