...
const getData = async (): Promise<Products> => {
    ...
    const addProducts = async (pageNum: number): Promise<Products> => {
        ...
        try {
            ...
            const response = await page.goto(`https://www.gianttiger.com/collections/flyers-and-deals?view=list&p=${pageNum}&L3_category_en=Bacon+Sausages+%26+Hot+Dogs&L3_category_en=Baking+Essentials&L3_category_en=Bread&L3_category_en=Breakfast&L3_category_en=Butter+%26+Spread&L3_category_en=Cakes+%26+Dessert&L3_category_en=Canned+%26+Pickled&L3_category_en=Chips+%26+Popcorn&L3_category_en=Coffee+%26+Tea&L3_category_en=Condiments+%26+Sauce&L3_category_en=Cookies+%26+Crackers&L3_category_en=Deli+Meat&L3_category_en=Fresh+Fruits&L3_category_en=Frozen+Meals+%26+Sides&L3_category_en=Frozen+Meat+%26+Seafood&L3_category_en=Frozen+Pizza&L3_category_en=Ice+Cream+%26+Desserts&L3_category_en=Milk+%26+Cream&L3_category_en=Oils+Vinegars+%26+Spices&L3_category_en=Pasta+%26+Pasta+Sauce&L3_category_en=Soups&store_inventory=bopis&online_inventory=sth`)
            ...
            // if current page is 1, then there will be a popup, 
            // wait for it to appear and close it
            if (pageNum === 1) {
                // wait for the close button of the popup selector
                await page.waitForSelector("#shopify-section-newsletter-popup #newsletter-close", {
                    visible: true
                })
                // click on the button once it is loaded, and wait for the main content
                await page.click("#shopify-section-newsletter-popup #newsletter-close") 
            }
            ...
            if (!collectionProducts) return products
            ...
            const currentProducts = await page.$$eval("div.product-tile__inner", (divProducts, getProductCategory__funcBody) => {
            ...
            console.error("Something went wrong (Giant Tiger): " + err.message)
