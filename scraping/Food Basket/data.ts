/*
    NOTE: 
    - This store is currently called 'Food and Things' -> product data fetching is done from 
    'Food and Things' website
*/
...
const getData = async (): Promise<Products> => {
    ...
    const addProducts = async (): Promise<Products> => {
        try {
            ...
            const response = await page.goto(`https://www.ubereats.com/ca/store/soups-%26-things/e_N0RKFXV9aMuESuaXxu4Q`, {
            ...
            const currentProducts = await page.$$eval("div[data-testid=store-catalog-section-vertical-grid] ul li", (divProducts, getProductCategory__funcBody, unitsRegexString) => {
                ...
                return divProducts.flatMap(div => {
                    ...
                    const title = div.querySelector("span.fx.fq.fy.be.di.bg.dj.b1")?.textContent?.trim() ?? Unavailable.Title
                    // get the products's price
                    const primaryPrice = div.querySelector("span.fx.fq.fy.be.bf.g0.dj.b1")?.textContent?.trim() ?? Unavailable.PrimaryPrice
                    // get units match 
            ...
            console.error("Something went wrong (Food Basket): " + err.message)
