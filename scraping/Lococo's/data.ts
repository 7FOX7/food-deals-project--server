...
const getData = async (): Promise<Products> => {
    ...
    const addProducts = async (): Promise<Products> => {
        try {
            ...
            const response = await page.goto(`https://lococos.ca/specials`, {
                waitUntil: "load"
            })
            ...
            await page.waitForSelector("div.album div.container-flex", { visible: true })
            ...
            const currentProducts = await page.$$eval("div.album div.container-flex div.card", (divProducts, getProductCategory__funcBody) => {
                ...
                return divProducts.flatMap(div => {
                    ...
                    const title = div.querySelector("div.product-name")?.textContent?.trim() ?? Unavailable.Title
                    // get the products's price
                    let primaryPrice = div.querySelector("span.the-price")?.textContent?.trim()
                    // if there is a price, then append `$` to it, otherwise, get the placeholder
                    primaryPrice = primaryPrice ? "$" + primaryPrice : Unavailable.PrimaryPrice
                    // get the product's units
                    const units = div.querySelector("span.the-size")?.textContent?.replace(/\s+/g, " ").replace(/(\()|(\))/g, "").toLowerCase().trim() ?? Unavailable.Units
            ...
            console.error("Something went wrong (Lococo's): " + err.message)
