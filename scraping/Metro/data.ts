...
const getData = async (): Promise<Products> => {
    ...
    const addProducts = async (pageNum: number): Promise<Products> => {
        ...
        try {
            ...
            const response = await page.goto(`https://www.metro.ca/en/online-grocery/flyer-page-${pageNum}?sortOrder=relevance&filter=%3Arelevance%3Adeal%3AFlyer+%26+Deals`, {
            ...
            await page.waitForSelector("div.products-second-block")
            const currentProducts = await page.$$eval("div.products-second-block div.default-product-tile", (divProducts, getProductCategory__funcBody) => {
            ...
            console.error("Something went wrong (Metro): " + err.message)
