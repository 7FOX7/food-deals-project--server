...
const getData = async (): Promise<Products> => {
    ...
    const addProducts = async (pageNum: number): Promise<Products> => {
        ...
        try {
            ...
            const response = await page.goto(`https://parce.ca/store-c/colombia/page/${pageNum}/?orderby=popularity`, {
                waitUntil: "load"
            })
            ...
            const currentProducts = await page.$$eval("ul.products li.product", (divProducts, getProductCategory__funcBody, unitsRegexString) => {
                ...
                return divProducts.flatMap(div => {
                    // get the title
                    const title = div.querySelector("h2.woocommerce-loop-product__title")?.textContent?.trim() ?? Unavailable.Title
                    // will be storing primary price
                    let primaryPrice: string = ""
                    // check if the price container (which contains the sale price) is present
                    if (div.querySelector("span.price ins") !== null) {
                        // get the sale price
                        primaryPrice = div.querySelector("span.price ins span.woocommerce-Price-amount bdi")?.textContent?.trim() ?? Unavailable.PrimaryPrice
                    } else {
                        // get the regular price
                        primaryPrice = div.querySelector("span.price span.woocommerce-Price-amount bdi")?.textContent?.trim() ?? Unavailable.PrimaryPrice
                    }
                    // get units match 
            ...
            console.error("Something went wrong (Patty's Delights Peruvian Culinary): " + err.message)
