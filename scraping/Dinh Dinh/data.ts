/*
    ...
    - `data` is extracted from Zehrs store (section East Asian food) because there is no catalog with products for `Dinh Dinh Asian Foods` store
*/
...
const getData = async (): Promise<Products> => {
    ...
    const addProducts = async (pageNum: number): Promise<Products> => {
        ...
        try {
            ...
            const response = await page.goto(`https://www.zehrs.ca/en/food/international-foods/east-asian-foods/c/58466?page=${pageNum}`, {
            ...
            console.error("Something went wrong (Dinh Dinh Asian Foods): " + err.message)
