...
const getData = async (): Promise<Products> => {
    ...
    const addProducts = async (): Promise<Products> => {
        try {
            ...
            const response = await page.goto(`https://www.skipthedishes.com/raja-grocers-lundys-lane`, {
            ...
            const currentProducts = await page.$$eval("div.sc-fUnMCh", (divProducts, getProductCategory__funcBody, unitsRegexString) => {
                ...
                return divProducts.flatMap(div => {
                    ...
                    const title = div.querySelector("h3.sc-87c0b655-1")?.textContent?.trim() ?? Unavailable.Title
                    // get the products's price
                    const primaryPrice = div.querySelector("h4.sc-87c0b655-3")?.textContent?.trim() ?? Unavailable.PrimaryPrice
                    // get units match 
            ...
            console.error("Something went wrong (Raja Grocers): " + err.message)
