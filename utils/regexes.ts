...
// (kg|g|ea|ml|l|gm|pk|cnt|packs?|lb|litre|piece) - the pattern should match one of those words
...
export const unitsRegex = /\b(\d+(\s+)?(\.|\-|x)(\s+)?)?(\d+)(\s+)?(\/(\s+)?)?(kg|g|ea|ml|l|gm|pk|cnt|packs?|lb|litre|piece)\b/ig